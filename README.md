...
uv run parse_incidents.py INPUT [INPUT ...] [options]
  ...
  --engine ENGINE       PDF text back-end: pdfium (default, pypdfium2 — several
                        times faster at ~the same memory) or pdfplumber (the
                        pure-Python fallback/reference). Both feed identical
